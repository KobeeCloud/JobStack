  ...
  return nodes.map(node => {
    ...
    switch (direction) {
      ...
      case 'LR':
        ...
        y =
          indexInLayer * (NODE_HEIGHT + nodeSpacing) -
          (nodesInLayer.length * (NODE_HEIGHT + nodeSpacing)) / 2 +
          300
        break
      case 'RL':
...
export function centerLayout(nodes: Node[], viewportWidth: number, viewportHeight: number): Node[] {
  if (nodes.length === 0) return []
