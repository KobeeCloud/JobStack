...
export function generateTerraform(nodes: Node[], edges: Edge[]): TerraformOutput[] {
  const outputs: TerraformOutput[] = []
  const providers = new Set<string>()
  const resources: string[] = []

  // Group nodes by provider
  const nodesByProvider: Record<string, Node[]> = {}
  ...
  nodes.forEach(node => {
    ...
    const resourceName = node.data.label.toLowerCase().replace(/[^a-z0-9]/g, '_')
...
export function generateTerraformReadme(nodes: Node[]): string {
...
function calculateTotalCost(nodes: Node[]): { min: number; max: number } {
  let min = 0
  let max = 0
