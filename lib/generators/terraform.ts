...
import { uniqueName, detectCycles } from '@/lib/generators/core/graph-utils'
...
function getAzureRgRef(
  ...
  if (rgNode) {
    const rgName =
      nodeIdToTfName.get(rgNode.id) || toTfName(String(rgNode.data?.label || 'resource_group'))
    return `azurerm_resource_group.${rgName}.name`
  }
  return 'var.azure_resource_group'
}

function getAzureVnetRef(
  node: Node<NodeData>,
  nodeMap: NodeMap,
  nodeIdToTfName: Map<string, string>
): string | null {
  ...
  if (vnetNode) {
    const vnetName =
      nodeIdToTfName.get(vnetNode.id) || toTfName(String(vnetNode.data?.label || 'vnet'))
    return `azurerm_virtual_network.${vnetName}.name`
  }
  return null
}

function getAzureSubnetRef(
  node: Node<NodeData>,
  nodeMap: NodeMap,
  nodeIdToTfName: Map<string, string>
): string | null {
  ...
  if (subnetNode) {
    const subnetName =
      nodeIdToTfName.get(subnetNode.id) || toTfName(String(subnetNode.data?.label || 'subnet'))
...
function generateImplicitNic(
  ...
  const hasExplicitNic = Array.from(nodeMap.values()).some(n => {
    ...
    return comp?.terraform?.resource === 'azurerm_network_interface' && n.parentId === vmNode.id
  })
...
const AZURE_EXPLICIT_KEYS = new Set([
  'resource_group_name',
    ...
    if (compId === 'azure-vm') {
      ...
      if (!subnetAncestor && !hasExplicitNic) {
        errors.push({
          ...
          error:
            'Placement Error: VM is not placed inside a Subnet, and no explicit NIC is attached.',
        })
      }
    }
...
export function generateTerraformWithValidation(
      ...
      errors: [
        {
          nodeId: '',
          nodeLabel: 'Diagram',
          error: 'No components found. Add components to generate Terraform.',
        },
      ],
      warnings: [],
      skippedCount: 0,
  ...
  nodes.forEach(node => {
    ...
    if (!componentId) {
      errors.push({
        nodeId: node.id,
        nodeLabel: String(node.data?.label || 'Unknown'),
    ...
    if (!component) {
      errors.push({
        nodeId: node.id,
        nodeLabel: String(node.data?.label || componentId),
          ...
          : [
              {
                nodeId: '',
                nodeLabel: 'Diagram',
                error: 'No components with Terraform support. Add AWS/Azure/GCP components.',
              },
            ],
      warnings,
      skippedCount,
  ...
  for (const edge of edges || []) {
    ...
    if (!srcExists) {
      ...
      warnings.push(
        `Edge to "${tgtLabel}" has a source node (${edge.source}) that was skipped or is unknown.`
      )
    }
    if (!tgtExists) {
      const srcLabel = nodeMap.get(edge.source)?.data?.label || edge.source
      warnings.push(
        `Edge from "${srcLabel}" has a target node (${edge.target}) that was skipped or is unknown.`
      )
    }
  }
  ...
  if (providers.has('aws'))
    mainTf += '    aws        = { source = "hashicorp/aws",             version = "~> 5.0" }\n'
  if (providers.has('gcp'))
    mainTf += '    google     = { source = "hashicorp/google",          version = "~> 5.0" }\n'
  if (providers.has('azure'))
    mainTf += '    azurerm    = { source = "hashicorp/azurerm",         version = "~> 3.0" }\n'
  if (providers.has('vercel'))
    mainTf += '    vercel     = { source = "vercel/vercel",             version = "~> 1.0" }\n'
  if (providers.has('cloudflare'))
    mainTf += '    cloudflare = { source = "cloudflare/cloudflare",    version = "~> 4.0" }\n'
  mainTf += '  }\n}\n\n'
  ...
  if (providers.has('gcp'))
    tfvars += `gcp_project  = "<your-gcp-project-id>"\ngcp_region   = "europe-west1"\n`
  if (providers.has('azure'))
    tfvars += `azure_location       = "West Europe"\nazure_resource_group = "rg-${projectName}-${environment}"\n`
  ...
  validNodes.forEach(node => {
    ...
    const safeName = uniqueName(
      String(node.data?.label || component.name),
      'terraform',
      issuedNames
    )
  ...
  validNodes.forEach(node => {
    ...
    if (isLinuxVm || isWindowsVm) {
      ...
      if (!img) {
        warnings.push(
          `${node.data?.label || resourceName}: Unknown OS image "${osImage}"; defaulting to Ubuntu 22.04`
        )
      }
    ...
    const supportsTags =
      ...
      resourceType.startsWith('google_')
  ...
  nodes
    ...
    .forEach(attachNode => {
      ...
      const attachName =
        nodeIdToTfName.get(attachNode.id) ||
        toTfName(String(attachNode.data?.label || attachCompId))
      const parentName =
        nodeIdToTfName.get(parentNode.id) ||
        toTfName(String(parentNode.data?.label || parentCompId))
  ...
  ;(edges || []).forEach(edge => {
    ...
    const srcName =
      nodeIdToTfName.get(srcNode.id) || toTfName(String(srcNode.data?.label || srcCompId))
    const tgtName =
      nodeIdToTfName.get(tgtNode.id) || toTfName(String(tgtNode.data?.label || tgtCompId))
    ...
    if (
      ...
      (tgtResource === 'azurerm_linux_virtual_machine' ||
        tgtResource === 'azurerm_windows_virtual_machine')
    ) {
  ...
  validNodes.forEach(node => {
    ...
    const component = getComponentById(componentId)!
    const resourceName =
      nodeIdToTfName.get(node.id) || toTfName(String(node.data?.label || component.name))
    const resourceType = component.terraform!.resource
...
function calculateTotalCost(nodes: Node<NodeData>[]): { min: number; max: number } {
  ...
  nodes.forEach(node => {
    ...
    if (id) {
      ...
      if (c) {
        min += c.estimatedCost.min
        max += c.estimatedCost.max
      }
    }
  })
  return { min, max }
