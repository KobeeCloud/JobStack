...
function convertToYaml(obj: unknown, indent = 0): string {
  ...
  if (typeof obj === 'string') {
    ...
    if (obj.includes('\n') || obj.includes(':') || obj.includes('#') || obj.includes('"') || obj.includes("'") || obj.includes('{') || obj.includes('}') || obj.includes('[') || obj.includes(']') || obj.includes('&') || obj.includes('*') || obj.includes('!') || obj.includes('|') || obj.includes('>') || obj.includes('%') || obj.includes('@')) {
  ...
  if (typeof obj === 'object') {
    ...
    if ('Fn::GetAtt' in obj) return `!GetAtt ${((obj as Record<string, unknown>)['Fn::GetAtt'] as string[]).join('.')}`
    if ('Fn::Join' in obj) {
      const [sep, parts] = (obj as Record<string, unknown>)['Fn::Join'] as [string, unknown[]]
      return `!Join\n${spaces}  - "${sep}"\n${spaces}  - ${convertToYaml(parts, indent + 2)}`
    }
    if ('Fn::Select' in obj) return `!Select ${convertToYaml((obj as Record<string, unknown>)['Fn::Select'], indent + 1)}`
