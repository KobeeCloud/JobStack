...
export function getGoogleConfig(clientId: string, redirectUri: string): SSOConfig {
  return {
    provider: 'google',
    enabled: true,
    clientId,
...
export function getOktaConfig(clientId: string, domain: string, redirectUri: string): SSOConfig {
...
export function getGitHubConfig(clientId: string, redirectUri: string): SSOConfig {
  return {
    provider: 'github',
    enabled: true,
    clientId,
...
export function generateAuthorizationUrl(config: SSOConfig, state: string, nonce?: string): string {
  ...
  const endpoint = config.metadata?.authorizationEndpoint || getDefaultAuthEndpoint(config.provider)
...
export function mapGroupsToRoles(groups: string[], mappings: RoleMapping[]): string[] {
...
export async function getSession(supabaseClient: ReturnType<typeof createClient>) {
...
export function shouldRefreshSession(session: SSOSession, thresholdMinutes: number = 5): boolean {
