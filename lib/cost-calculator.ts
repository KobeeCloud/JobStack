...
function calculateNodeCost(node: Node): { min: number; max: number } | null {
  ...
  if (
    ['azure-app-service', 'azure-functions', 'aws-lambda', 'gcp-cloud-functions'].includes(
      componentId
    )
  ) {
  ...
  // ── Storage accounts ──
  if (
    ['azure-storage-account', 'azure-blob', 'aws-s3', 'gcp-cloud-storage'].includes(componentId)
  ) {
  ...
  if (
    config.replicas &&
    !['azure-vm', 'aws-ec2', 'gcp-compute-instance', 'azure-vmss'].includes(componentId)
  ) {
    const replicas = Number(config.replicas) || 1
    if (replicas > 1) {
      baseCostMin *= replicas
