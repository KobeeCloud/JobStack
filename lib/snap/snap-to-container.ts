...
export function calculateSnapPosition(
  ...
  for (const container of containers) {
    ...
    if (container.acceptedTypes.length > 0 && !container.acceptedTypes.includes(nodeType)) {
      continue
    }
  ...
  if (bestContainer) {
    ...
    snappedPosition = {
      ...
      y: Math.max(
        bestContainer.top,
        Math.min(bestContainer.bottom - nodeHeight, snappedPosition.y)
      ),
    }
...
export function useSnapToContainer(nodes: Node[], config: Partial<SnapConfig> = {}) {
  const mergedConfig = useMemo(() => ({ ...DEFAULT_SNAP_CONFIG, ...config }), [config])
  ...
  const getContainerHighlights = useCallback(
    (draggedNode: Node): Array<{ id: string; isValid: boolean }> => {
      ...
      return containers.map(container => {
        ...
        const isNearby =
          getDistanceToBounds(nodeCenter, container.bounds) < mergedConfig.containerSnapThreshold
        const isAccepted =
          container.bounds.acceptedTypes.length === 0 ||
          container.bounds.acceptedTypes.includes(nodeType)
...
export function reparentNode(node: Node, newParentId: string | null, nodes: Node[]): Node[] {
  return nodes.map(n => {
    if (n.id === node.id) {
      if (newParentId) {
