...
export function DrawioImportDialog({ open, onOpenChange, onImport }: DrawioImportDialogProps) {
            ...
            >
              ...
              {state === 'error' ? (
                ...
                <>
                  ...
                  />
                  <p className="text-gray-600 mb-2">Drag and drop your draw.io file here, or</p>
                  <label
                  ...
                  <p className="text-xs text-gray-400 mt-2">Supports .drawio and .xml files</p>
                </>
              )}
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 p-4 bg-green-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600">Ready to import your diagram</p>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{result.metadata.importedNodes}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Edges:</span>
                    <span className="font-medium">{result.metadata.importedEdges}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Total cells:</span>
                    <span className="font-medium">{result.metadata.totalCells}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Skipped:</span>
                    <span className="font-medium">{result.metadata.skippedCells}</span>
                  </div>
                </div>
              </div>
              ...
              {result.nodes.length > 0 && (
                <div className="text-xs text-gray-500">
                  ...
                  {[...new Set(result.nodes.map(n => n.type || 'default'))].slice(0, 5).join(', ')}
                  {new Set(result.nodes.map(n => n.type)).size > 5 &&
                    ` and ${new Set(result.nodes.map(n => n.type)).size - 5} more`}
                </div>
              )}
            </div>
          )}
