...
const PATTERNS: QuickBuildPattern[] = [
  // ── Azure ──────────────────────────────────────────────────────────────────
  {
    ...
    description:
      'Resource Group → VNet → Subnet → NSG + N × VMs. Perfect starting point for lift-and-shift migrations.',
    icon: '🖥️',
    provider: 'azure',
    tags: ['VM', 'VNet', 'IaaS'],
    params: [
    ...
    nodes: [
      ...
      },
      {
        ...
        x: 80,
        y: 120,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['{{subnetCidr}}'] },
      },
      {
        id: 'nsg',
      ...
      },
      {
        ...
        x: 50,
        y: 80,
        parentTemplateId: 'subnet',
        repeat: 'vmCount',
        repeatOffsetX: 160,
        repeatOffsetY: 0,
        config: { size: 'Standard_D2s_v3' },
      },
    ],
    edges: [],
  ...
  },
  {
    ...
    description:
      'Dedicated VNet with system + user node pool subnets, AKS cluster, and Azure Container Registry with private endpoint.',
    icon: '☸️',
    provider: 'azure',
    ...
    nodes: [
      ...
      },
      {
        ...
        y: 130,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['10.10.1.0/24'] },
      },
      {
        id: 'userSubnet',
        componentId: 'azure-subnet',
        label: 'user-nodepool-subnet',
        category: 'networking',
        provider: 'azure',
        isContainer: true,
        width: 330,
        ...
        y: 130,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['10.10.2.0/24'] },
      },
      {
        id: 'aks',
    ...
    edges: [{ from: 'aks', to: 'acr' }],
  },
  {
    id: 'azure-3tier',
    name: 'Azure – 3-Tier Secure Web App',
    description:
      'App Gateway (DMZ) → Web tier → App tier → DB tier. NSG on each subnet, Azure SQL in the backend.',
    icon: '🏛️',
    provider: 'azure',
    ...
    nodes: [
      ...
      },
      {
        ...
        y: 120,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['10.20.0.0/26'] },
      },
      {
        id: 'subWeb',
        componentId: 'azure-subnet',
        ...
        y: 120,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['10.20.1.0/24'] },
      },
      {
        id: 'subApp',
        componentId: 'azure-subnet',
        ...
        y: 320,
        parentTemplateId: 'vnet',
        config: { address_prefixes: ['10.20.2.0/24'] },
      },
      {
        id: 'subDb',
        componentId: 'azure-subnet',
      ...
      },
      {
        id: 'vm1',
        componentId: 'azure-vm',
        label: '{{prefix}}-web-vm1',
        category: 'compute',
        provider: 'azure',
        x: 200,
        y: 150,
        parentTemplateId: 'subWeb',
      },
      {
        id: 'vm2',
        componentId: 'azure-vm',
        label: '{{prefix}}-app-vm1',
        category: 'compute',
        provider: 'azure',
        x: 200,
        y: 350,
        parentTemplateId: 'subApp',
      },
      {
        id: 'sql',
  ...
  // ── AWS ────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      'VPC with public + private subnets across 2 AZs, IGW, NAT GW, ALB, Security Group, and N × EC2 instances.',
    icon: '⚙️',
    provider: 'aws',
    ...
    nodes: [
      {
        ...
        x: 50,
        y: 50,
        config: { cidr_block: '{{cidr}}', enable_dns_hostnames: true },
      },
      {
        id: 'pubSub',
        componentId: 'aws-subnet',
      ...
      },
      {
        ...
        y: 100,
        parentTemplateId: 'vpc',
        config: { cidr_block: '10.0.2.0/24', availability_zone: 'a' },
      },
      {
        id: 'igw',
        componentId: 'aws-internet-gateway',
        label: '{{prefix}}-igw',
        category: 'networking',
        provider: 'aws',
        x: 80,
  ...
  },
  {
    ...
    description:
      'API Gateway → Lambda (Node.js 20) → DynamoDB. Zero-infrastructure, pay-per-request pattern.',
    icon: '⚡',
    provider: 'aws',
    tags: ['Lambda', 'Serverless', 'API Gateway'],
    params: [{ key: 'prefix', label: 'Name prefix', type: 'text', default: 'api' }],
    nodes: [
      {
        id: 'apigw',
      ...
      },
      {
        ...
        label: '{{prefix}}-handler',
        category: 'appservices',
        provider: 'aws',
        x: 400,
        y: 200,
        config: { runtime: 'nodejs20.x', architectures: ['arm64'], timeout: 30 },
      },
      {
        id: 'dynamo',
  ...
  },
  {
    ...
    description:
      'VPC with 2 private subnets across 2 AZs, EKS cluster, ECR registry, and ALB Ingress.',
    icon: '🐳',
    provider: 'aws',
  ...
  // ── GCP ────────────────────────────────────────────────────────────────────
  {
    ...
    description:
      'VPC → Subnet → Firewall rules + N × Compute Instances behind a Cloud Load Balancer.',
    icon: '🌐',
    provider: 'gcp',
    ...
    nodes: [
      ...
      {
        id: 'subnet',
        componentId: 'gcp-subnet',
        label: '{{prefix}}-subnet ({{cidr}})',
        category: 'networking',
        provider: 'gcp',
        isContainer: true,
        width: 700,
        ...
        y: 100,
        parentTemplateId: 'vpc',
        config: { private_ip_google_access: true },
      },
      {
        id: 'fw',
        componentId: 'gcp-firewall',
        label: '{{prefix}}-fw-allow-http',
        category: 'networking',
        provider: 'gcp',
        nodeType: 'attachment',
        x: 80,
      ...
      },
      {
        ...
        y: 450,
        parentTemplateId: 'vpc',
        config: { load_balancing_scheme: 'EXTERNAL_MANAGED', protocol: 'HTTPS' },
      },
    ],
    edges: [
      { from: 'lb', to: 'vm1' },
  ...
  },
  {
    ...
    description:
      'Dedicated VPC + subnet with Private Google Access, GKE Autopilot cluster, and Cloud Storage bucket for artifacts.',
    icon: '☸️',
    provider: 'gcp',
    ...
    nodes: [
      {
        ...
        x: 50,
        y: 50,
        config: { routing_mode: 'REGIONAL' },
      },
      {
        id: 'subnet',
        componentId: 'gcp-subnet',
        label: '{{prefix}}-nodes-subnet',
        category: 'networking',
        provider: 'gcp',
        isContainer: true,
        width: 700,
        ...
        y: 100,
        parentTemplateId: 'vpc',
        config: { private_ip_google_access: true, purpose: 'PRIVATE', log_config_enable: true },
      },
      {
        id: 'fw',
        componentId: 'gcp-firewall',
        label: '{{prefix}}-fw-internal',
        category: 'networking',
        provider: 'gcp',
        nodeType: 'attachment',
        x: 80,
        y: 430,
        parentTemplateId: 'vpc',
        config: { direction: 'INGRESS', source_tags: ['k8s-node'] },
      },
      {
        id: 'gke',
        componentId: 'gcp-gke',
      ...
      },
      {
        ...
        label: '{{prefix}}-artifacts',
        category: 'storage',
        provider: 'gcp',
        x: 650,
        y: 430,
        parentTemplateId: 'vpc',
        config: { storage_class: 'STANDARD', uniform_bucket_level_access: true },
      },
    ],
    edges: [{ from: 'gke', to: 'gcs' }],
  },
]
...
function buildNodesAndEdges(
  ...
  const nodes: Node[] = expandedTemplates.map((tmpl, idx) => {
    ...
    const config = tmpl.config
      ? (interpolateDeep(tmpl.config, params) as Record<string, unknown>)
      : {}
              ...
              <div className="p-3 space-y-1">
                {(['azure', 'aws', 'gcp'] as const).map(provider => {
                  ...
                  return (
                    <div key={provider}>
                      <div
                        className={`text-xs font-semibold uppercase tracking-wider px-2 py-1.5 rounded ${PROVIDER_STYLES[provider]}`}
                      >
                        {provider === 'azure'
                          ? 'Microsoft Azure'
                          : provider === 'aws'
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`text-xs font-medium px-2 py-0.5 rounded-full ${PROVIDER_STYLES[selectedPattern.provider]}`}
                    >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {selectedPattern.nodes.map(n => (
                      ...
                      >
                        ...
                        {n.isContainer && (
                          <span className="ml-auto text-muted-foreground">[container]</span>
                        )}
                      </div>
                    ))}
                  </div>
                  {(selectedPattern.edges?.length ?? 0) > 0 && (
                    <p className="text-xs text-muted-foreground mt-1.5">
                      + {selectedPattern.edges!.length} connection
                      {selectedPattern.edges!.length > 1 ? 's' : ''} between components
                    </p>
                  )}
                </div>
