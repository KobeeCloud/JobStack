...
export const GET = createApiHandler(
  async (request: NextRequest, { auth }) => {
    ...
    if (!membership) {
      throw new ApiError(403, 'You must be a member of this organization', 'FORBIDDEN')
    }
...
export const POST = createApiHandler(
  async (request: NextRequest, { auth }) => {
    ...
    if (count !== null && count >= 100) {
      throw new ApiError(400, 'Maximum 100 custom components per organization', 'COMPONENT_LIMIT')
    }
